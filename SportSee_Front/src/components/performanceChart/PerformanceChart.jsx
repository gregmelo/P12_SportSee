...
export default function PerformanceChart({ performanceData }) {
  ...
  const formattedData = performanceData.data.map((item) => ({
    kind: kindTranslations[performanceData.kind[item.kind]], // Associe l'index à son label
    value: item.value,
  }));
  ...
  return (
    ...
    <RadarChart outerRadius="80%" cx="50%" cy="50%" data={formattedData}>
      ...
      <PolarAngleAxis dataKey="kind" tick={{ fill: '#ffffff', fontSize: 10 }}/> {/* Utilise les labels */}
